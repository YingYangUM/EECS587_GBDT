...
__global__ void get_err(int* A, int* B, float* C, float* D, float* E, float* out, int pre, int N){

    int threadId = threadIdx.x + blockIdx.x*blockDim.x;
    if(threadId < N && N >= pre){
        int cnt0 = A[threadId];
        int cnt = B[threadId];
        float ss_y = C[threadId];
        float sum_y = D[threadId];
        float sum0 = E[threadId];
        if (cnt0 >= 10 && (cnt-cnt0 >= 10))
            out[threadId] =  ss_y - sum0 * sum0 / cnt0 - (sum_y-sum0) * (sum_y-sum0) / (cnt-cnt0);
        else
            out[threadId] = - 1.0;
    }
}
...
typedef union  {
    float floats[2];                 // floats[0] = minvalue
    int ints[2];                     // ints[1] = minindex
    unsigned long long int ulong;    // for atomic update
} my_atomics;

__device__ unsigned long long int get_atomicMin(unsigned long long int* address, float val1, int val2)
{
...
__global__ void get_min_idx(const float *data, const int *keys,const int N, my_atomics *res)
{

    int idx = (blockDim.x * blockIdx.x) + threadIdx.x;
    if (idx < N && keys[idx] >= 0 && data[idx] >= 0)
        get_atomicMin(&(res[keys[idx]].ulong), data[idx], idx);
}
...
class DecisionTree {
    ...
    // used in find_split
    thrust::device_vector<int> ordered_index;
    thrust::device_vector<int> ordered_pos;
    thrust::device_vector<float> ordered_fea;
    thrust::device_vector<float> ordered_y;
    thrust::device_vector<float> ordered_sqr_y;
    thrust::device_vector<float> ordered_err;

    thrust::device_vector<int> counts0;
    thrust::device_vector<int> counts;
    thrust::device_vector<float> flat_ss_y;
    thrust::device_vector<float> flat_sum_y;
            ...
            vector<ThreadInfo> &tinfo_list)
    {
        ...
        int pre = 0;
        thrust::device_vector<int> node_counts(n_q);
        thrust::device_vector<float> node_sum_y(n_q);
        thrust::device_vector<float> node_sum_sqr_y(n_q);
        for (int i = 0; i < n_q; i++) {
            tinfo_list[i].cnt0 = 0;        
            ...
            node_counts[i] = cnt;
            node_sum_y[i] = sum_y;
            node_sum_sqr_y[i] = sum_sqr_y;
            pre +=  q[i].cnt;
        }
        pre = N - pre; // number of events in -1 position

        double start_time = get_time();

        thrust::gather(index_list.begin(), index_list.end(), positions.begin(), ordered_pos.begin());

        int numElements = index_list.size();
        thrust::counting_iterator<int> iter(0);
        thrust::device_vector<int> indices(numElements);
        thrust::copy(iter, iter + indices.size(), indices.begin());

        thrust::stable_sort_by_key(ordered_pos.begin(), ordered_pos.end(), indices.begin());
        
        // use stable sort to sort the feature again according to the pos
        thrust::gather(indices.begin(), indices.end(), index_list.begin(), ordered_index.begin());
        thrust::gather(indices.begin(), indices.end(), fea_list.begin(), ordered_fea.begin());

        thrust::gather(ordered_index.begin(), ordered_index.end(), y_list.begin(), ordered_y.begin());
        thrust::gather(ordered_index.begin(), ordered_index.end(), sqr_y_list.begin(), ordered_sqr_y.begin());

        thrust::exclusive_scan_by_key(ordered_pos.begin(), ordered_pos.end(), ordered_y.begin(), ordered_y.begin());
        thrust::exclusive_scan_by_key(ordered_pos.begin(), ordered_pos.end(), ordered_sqr_y.begin(), ordered_sqr_y.begin());

        // calculate the err
        thrust::fill(counts0.begin(), counts0.end(), 1);
        thrust::exclusive_scan_by_key(ordered_pos.begin(), ordered_pos.end(), counts0.begin(), counts0.begin());

        // get counts1 and sum_sqr_y for each node
        thrust::gather(ordered_pos.begin()+pre, ordered_pos.end(), node_counts.begin(), counts.begin()+pre);
        thrust::gather(ordered_pos.begin()+pre, ordered_pos.end(), node_sum_y.begin(), flat_sum_y.begin()+pre);
        thrust::gather(ordered_pos.begin()+pre, ordered_pos.end(), node_sum_sqr_y.begin(), flat_ss_y.begin()+pre);

        // counts & ordered_y -> ordered_err;
        int grid_size = (N+BLOCK_SIZE-1)/BLOCK_SIZE;
        get_err<<<grid_size, BLOCK_SIZE>>>(thrust::raw_pointer_cast(counts0.data()),
                                           thrust::raw_pointer_cast(counts.data()),
                                           thrust::raw_pointer_cast(flat_ss_y.data()),
                                           thrust::raw_pointer_cast(flat_sum_y.data()),
                                           thrust::raw_pointer_cast(ordered_y.data()),
                                           thrust::raw_pointer_cast(ordered_err.data()),
                                           pre, N);


        my_atomics *results;
        cudaMalloc(&results, n_q*sizeof(my_atomics));
        cudaMemset(results, 0x7f, n_q*sizeof(my_atomics));
        // cudaMemcpy(results, h_results, n_q*sizeof(my_atomics), cudaMemcpyHostToDevice);

        get_min_idx<<<grid_size, BLOCK_SIZE>>>(thrust::raw_pointer_cast(ordered_err.data()), thrust::raw_pointer_cast(ordered_pos.data()), N, results);
        cudaDeviceSynchronize();

        my_atomics *h_results = new my_atomics[n_q];
        cudaMemcpy(h_results, results, n_q*sizeof(my_atomics), cudaMemcpyDeviceToHost);

        printf("%.3f seconds for cuda running\n", get_time() - start_time);
        ...
        for(int i = 0; i < n_q; i++){
            ...
            if (cnt < 20) {
                pre += cnt;
                continue; // don't have enough children
            }
            ...
            unsigned int index = h_results[i].ints[1];
            float err = h_results[i].floats[0];

            if (sign(err - tinfo.spinfo.err) < 0) {                        
                SplitInfo &tbest = tinfo.spinfo;
                tbest.err = err;
                tbest.bind = fid;                    
                tbest.bsplit = (ordered_fea[index-1] + ordered_fea[index]) / 2;

                tbest.sum_y[0] = ordered_y[index];
                tbest.sum_y[1] = sum_y - ordered_y[index];

                tbest.sum_sqr_y[0] = ordered_sqr_y[index];
                tbest.sum_sqr_y[1] = sum_sqr_y - ordered_sqr_y[index];

                tbest.cnt[0] = index - pre;
                tbest.cnt[1] = cnt - index + pre;
            }
    ...
    DecisionTree(vector<DFeature> &features, int max_depth, int max_feature, int max_pos, 
        ...
        ordered_index.resize(n);
        ordered_pos.resize(n);
        ordered_fea.resize(n);
        ordered_y.resize(n);
        ordered_sqr_y.resize(n);
        ordered_err.resize(n);
